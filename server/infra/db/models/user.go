...
type userR struct {
	...
	ScheduledTasks ScheduledTaskSlice `boil:"ScheduledTasks" json:"ScheduledTasks" toml:"ScheduledTasks" yaml:"ScheduledTasks"`
	ShareTasks     ShareTaskSlice     `boil:"ShareTasks" json:"ShareTasks" toml:"ShareTasks" yaml:"ShareTasks"`
}
...
func (r *userR) GetBaseTasks() BaseTaskSlice {
	if r == nil {
		return nil
	}
	return r.BaseTasks
...
func (r *userR) GetScheduledTasks() ScheduledTaskSlice {
	if r == nil {
		return nil
	}
...
func (r *userR) GetShareTasks() ShareTaskSlice {
	if r == nil {
		return nil
	}
	return r.ShareTasks
...
var (
	userAllColumns            = []string{"id", "uuid", "display_name", "created_at", "updated_at"}
	userColumnsWithoutDefault = []string{"uuid", "display_name", "created_at", "updated_at"}
...
func (o *User) BaseTasks(mods ...qm.QueryMod) baseTaskQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`base_task`.`user_id`=?", o.ID),
	)

	return BaseTasks(queryMods...)
}

// ScheduledTasks retrieves all the scheduled_task's ScheduledTasks with an executor.
func (o *User) ScheduledTasks(mods ...qm.QueryMod) scheduledTaskQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("`scheduled_task`.`user_id`=?", o.ID),
	)

	return ScheduledTasks(queryMods...)
}

// ShareTasks retrieves all the share_task's ShareTasks with an executor.
func (o *User) ShareTasks(mods ...qm.QueryMod) shareTaskQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("`share_task`.`user_id`=?", o.ID),
	)

	return ShareTasks(queryMods...)
}

// LoadBaseTasks allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadBaseTasks(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`base_task`),
		qm.WhereIn(`base_task.user_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// LoadScheduledTasks allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadScheduledTasks(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`scheduled_task`),
		qm.WhereIn(`scheduled_task.user_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load scheduled_task")
	}

	var resultSlice []*ScheduledTask
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice scheduled_task")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on scheduled_task")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for scheduled_task")
	}
	...
	if singular {
		object.R.ScheduledTasks = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &scheduledTaskR{}
			}
			foreign.R.User = object
		}
		return nil
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.UserID {
				local.R.ScheduledTasks = append(local.R.ScheduledTasks, foreign)
				if foreign.R == nil {
					foreign.R = &scheduledTaskR{}
				}
				foreign.R.User = local
...
// LoadShareTasks allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadShareTasks(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`share_task`),
		qm.WhereIn(`share_task.user_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load share_task")
	}

	var resultSlice []*ShareTask
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice share_task")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on share_task")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for share_task")
	}
	...
	if singular {
		object.R.ShareTasks = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &shareTaskR{}
			}
			foreign.R.User = object
		}
		return nil
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.UserID {
				local.R.ShareTasks = append(local.R.ShareTasks, foreign)
				if foreign.R == nil {
					foreign.R = &shareTaskR{}
				}
				foreign.R.User = local
...
func (o *User) AddBaseTasks(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*BaseTask) error {
	var err error
...
// AddScheduledTasks adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.ScheduledTasks.
// Sets related.R.User appropriately.
func (o *User) AddScheduledTasks(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*ScheduledTask) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.UserID = o.ID
	...
	if o.R == nil {
		o.R = &userR{
			ScheduledTasks: related,
		}
	} else {
		o.R.ScheduledTasks = append(o.R.ScheduledTasks, related...)
	}

	for _, rel := range related {
		if rel.R == nil {
			rel.R = &scheduledTaskR{
				User: o,
			}
		} else {
			rel.R.User = o
...
// AddShareTasks adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.ShareTasks.
// Sets related.R.User appropriately.
func (o *User) AddShareTasks(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*ShareTask) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.UserID = o.ID
	...
	if o.R == nil {
		...
		o.R.ShareTasks = append(o.R.ShareTasks, related...)
	}
