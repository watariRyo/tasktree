...
func testUserToManyBaseTasks(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := UserSlice{&a}
	if err = a.L.LoadBaseTasks(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testUserToManyScheduledTasks(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, scheduledTaskDBTypes, false, scheduledTaskColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, scheduledTaskDBTypes, false, scheduledTaskColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.ScheduledTasks().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadScheduledTasks(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.ScheduledTasks); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.ScheduledTasks = nil
	if err = a.L.LoadScheduledTasks(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.ScheduledTasks); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyShareTasks(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, shareTaskDBTypes, false, shareTaskColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, shareTaskDBTypes, false, shareTaskColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.ShareTasks().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadShareTasks(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.ShareTasks); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.ShareTasks = nil
	if err = a.L.LoadShareTasks(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.ShareTasks); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAddOpBaseTasks(t *testing.T) {
	var err error
...
func testUserToManyAddOpScheduledTasks(t *testing.T) {
	var err error
	...
	var a User
	var b, c, d, e ScheduledTask

	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, userDBTypes, false, strmangle.SetComplement(userPrimaryKeyColumns, userColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	foreigners := []*ScheduledTask{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, scheduledTaskDBTypes, false, strmangle.SetComplement(scheduledTaskPrimaryKeyColumns, scheduledTaskColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*ScheduledTask{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddScheduledTasks(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.ScheduledTasks[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.ScheduledTasks[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.ScheduledTasks().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
func testUserToManyAddOpShareTasks(t *testing.T) {
	var err error
	...
	var a User
	var b, c, d, e ShareTask

	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, userDBTypes, false, strmangle.SetComplement(userPrimaryKeyColumns, userColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	foreigners := []*ShareTask{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, shareTaskDBTypes, false, strmangle.SetComplement(shareTaskPrimaryKeyColumns, shareTaskColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*ShareTask{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddShareTasks(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.ShareTasks[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.ShareTasks[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.ShareTasks().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
var (
	userDBTypes = map[string]string{`ID`: `int`, `UUID`: `varchar`, `DisplayName`: `varchar`, `CreatedAt`: `datetime`, `UpdatedAt`: `datetime`}
	_           = bytes.MinRead
)
